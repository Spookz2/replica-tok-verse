...
const InboxScreen = () => {
  ...
  return (
    <div className="h-full bg-tiktok-black text-white overflow-y-auto">
      {/* Header */}
      <div className="p-4 border-b border-tiktok-gray-800">
      ...
      <div className="p-4">
        <div className="flex space-x-4 mb-6">
          ...
          <div className="flex flex-col items-center">
            <div className="w-16 h-16 bg-tiktok-gray-700 rounded-full flex items-center justify-center relative">
              <div className="w-6 h-6 bg-tiktok-blue rounded-full flex items-center justify-center absolute -bottom-1 -right-1">
          ...
          {conversations.map((conv) => (
            <div key={conv.id} className="flex flex-col items-center">
              <div className={`w-16 h-16 rounded-full p-0.5 ${conv.hasStory ? 'bg-gradient-to-r from-tiktok-pink to-tiktok-red' : ''}`}>
                ...
                {conv.isOnline && (
                  <div className="w-4 h-4 bg-green-500 rounded-full border-2 border-tiktok-black absolute -bottom-1 -right-1"></div>
                )}
              </div>
              <span className="text-xs mt-2 text-tiktok-gray-300 max-w-16 truncate">{conv.username}</span>
              <div className="bg-tiktok-gray-700 rounded-lg px-2 py-1 mt-1 max-w-20">
